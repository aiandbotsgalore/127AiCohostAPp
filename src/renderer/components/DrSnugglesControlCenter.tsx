...
// ⚡ Bolt: Isolated AudioMeter to prevent main component re-renders (60Hz)
const AudioMeterWidget: React.FC = () => {
    const [level, setLevel] = useState(0);

    useEffect(() => {
        // Subscribe to audio-level events directly
        const unsubscribe = ipc.on('audio-level', (_event: any, data: any) => {
            setLevel(data.level);
        });
        return unsubscribe;
    }, []);
...
const DrSnugglesControlCenter: React.FC = () => {
    ...
    // audioLevel state removed to prevent main component re-renders
...
You speak with ruthless brevity, two or three sentences at most, carved with surgical precision.`
    ...
    // audioLevel sync effect removed
    ...
    useEffect(() => {
        ...
        unsubscribers.push(ipc.on('audio-level', (event, data) => {
            audioLevelRef.current = data.level; // Update ref directly, no re-render
        }));
