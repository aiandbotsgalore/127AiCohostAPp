...
export const StatusBar: React.FC = () => {
    ...
    useEffect(() => {
        ...
        unsubscribers.push(ipc.on('genai:latencyUpdate', (event, data: { totalRoundtrip: number }) => {
            void event;
            setLatency(data.totalRoundtrip);
            setLatencyHistory(prev => [...prev, data.totalRoundtrip].slice(-30));
        }));

        unsubscribers.push(ipc.on('processing:status', (event, data: { queueDepth: number, processingDelay: number }) => {
            void event;
