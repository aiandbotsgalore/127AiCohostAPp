...
export const AvatarWidget: React.FC<AvatarWidgetProps> = React.memo(({ vadStatus, onStatusAction }) => {
    // State
    ...
    // Sync vadStatus to ref for animation loop
    useEffect(() => {
        vadStatusRef.current = vadStatus;
    }, [vadStatus]);

    // Listen for audio levels locally
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        // Set dimensions based on styles or fixed size matching original
        // Original was 200x200
    ...
    }, []); // Run on mount (when parent renders us)
