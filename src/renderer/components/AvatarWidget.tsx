...
export const AvatarWidget: React.FC<AvatarWidgetProps> = memo(({ vadStatus, onStatusAction }) => {
    // Local State for high-frequency animations
    ...
    // Refs for avoiding re-renders on high-frequency IPC updates
    const audioLevelRef = useRef(0);
    const vadStatusRef = useRef(vadStatus);

    // Sync vadStatus prop to ref for animation loop
    useEffect(() => {
        vadStatusRef.current = vadStatus;
    }, [vadStatus]);

    // IPC Listener for Audio Level (Local to this component)
    useEffect(() => {
